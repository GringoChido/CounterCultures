/**
 * Week 2 Day 3 — add `brand_slugs` column to the Leads tab in the CRM Sheet.
 *
 * Appends a single column at the end of the existing Leads header row.
 * Idempotent: if `brand_slugs` already exists (any column), this is a no-op.
 * Existing data rows are untouched — their brand_slugs cell remains blank.
 *
 * Run: npx tsx scripts/add-brand-slugs-to-leads.ts
 */
...
const main = async () => {
  ...
  if (existingIndex >= 0) {
    console.log(`✓ "${NEW_COLUMN}" already exists at column ${colLetter(existingIndex)} (index ${existingIndex}). No-op.`);
    return;
  }

  const nextCol = colLetter(row.length);
  console.log(`Appending "${NEW_COLUMN}" at column ${nextCol} (existing header length: ${row.length}).`);
