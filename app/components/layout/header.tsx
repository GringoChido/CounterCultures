...
import { NAV_LINKS, PRODUCT_CATEGORIES, SITE_CONFIG } from "@/app/lib/constants";
...
const WA_GREETINGS = {
  en: "Hi, I'd like to learn more about Counter Cultures [src=nav_utility]",
  es: "Hola, me interesa conocer más sobre Counter Cultures [src=nav_utility]",
} as const;
...
const Header = ({ transparent = false }: { transparent?: boolean }) => {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-16 md:h-20 lg:h-[116px] ${
          isTransparent
            ? "bg-transparent border-b border-transparent"
            : "bg-brand-linen/95 backdrop-blur-sm border-b border-brand-stone/10"
        }`}
        style={{ "--cc-header-h": "116px" } as React.CSSProperties}
      >
        {/* Row 1 — Utility strip (desktop only) */}
        <div className={`hidden lg:flex h-9 items-center border-b ${isTransparent ? "border-white/10" : "border-brand-stone/10"}`}>
          <div className="mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 flex items-center justify-end">
            <div className="flex items-center gap-0">
              <a
                href={phoneHref}
                className={`inline-flex items-center gap-1.5 min-h-[36px] px-3 font-body text-xs transition-colors ${
                  isTransparent
              ...
              <a
                ...
                className={`inline-flex items-center gap-1.5 min-h-[36px] px-3 font-body text-xs transition-colors ${
                  isTransparent
                    ? "text-white/70 hover:text-white"
                    : "text-dash-text-secondary hover:text-brand-terracotta"
                }`}
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 h-9 transition-colors cursor-pointer w-9 justify-center lg:w-56 lg:h-10 lg:justify-start lg:px-4 lg:border lg:rounded-full ${
                  isTransparent
                    ? "text-white hover:text-white/70 lg:border-white/25 lg:bg-white/10 lg:hover:border-white/40"
                    : "text-brand-charcoal hover:text-brand-terracotta lg:border-brand-stone/25 lg:bg-white/60 lg:hover:border-brand-stone/40"
                }`}
                aria-label={t("searchAriaLabel")}
              >
                <Search className="w-4 h-4 shrink-0" />
                <span className="hidden lg:inline font-body text-sm text-dash-text-secondary/70">
                  {t("search")}
                </span>
                <kbd className="hidden lg:inline font-mono text-[10px] text-dash-text-secondary/50 border border-brand-stone/20 rounded px-1.5 py-0.5 ml-auto">
                  ⌘K
                </kbd>
              </button>
          ...
          {mobileOpen && (
            ...
            >
              <div className="px-4 sm:px-6 py-4 space-y-0 max-h-[calc(100dvh-4rem)] overflow-y-auto">
                ...
                {/* Utility links — mobile parity */}
                <a
                  href={phoneHref}
                  className="flex items-center gap-3 py-3.5 min-h-[44px] font-body text-base font-medium text-brand-charcoal hover:text-brand-terracotta transition-colors border-b border-brand-stone/5"
                  aria-label={`${t("phoneLabel")} +52 ${phoneRaw}`}
                >
