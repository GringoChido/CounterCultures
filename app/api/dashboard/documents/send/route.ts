...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const {
      ...
      subject,
      message,
      // send-multi fields — single API call dispatches to both channels.
      // WhatsApp stays in dry-run mode (returns wa.me URL only) unless
      // WHATSAPP_ENABLED=true; no Cloud API integration yet.
      email,
      phone,
      emailSubject,
      emailMessage,
    ...
    if (action === "send-multi") {
      ...
      // Branch 2 — WhatsApp path. The client only passes `phone` when the
      // user has the "Also send via WhatsApp" toggle checked. Drive share
      // link is appended to the body so the customer can tap through.
      if (phone) {
        let shareLink = "";
        if (doc?.Drive_File_ID && isConfigured()) {
          try {
