...
import { createPurchaseOrder } from "@/app/lib/odoo/write";
...
export const POST = async (req: NextRequest): Promise<Response> => {
  try {
    const user = await requireFeature("create_po");
    ...
    appendRow("Activity_Log", [
      ...
      console.error("[purchase-orders/create] Activity_Log append failed:", err)
    );

    invalidateOdooCache("purchases");
    ...
    const msg = err instanceof Error ? err.message : "create_po_failed";
    console.error("[/api/dashboard/purchase-orders/create]", msg);
