...
export const getAllVendorTerms = async (): Promise<VendorTerms[]> =>
  Object.entries(TERMS_BY_VENDOR_LOWER).map(([key, terms]) => ({
    vendor: key,
    name: key.split(" ").map((w) => w[0].toUpperCase() + w.slice(1)).join(" "),
    terms,
    depositPercent: terms === "advance" ? 100 : undefined,
  }));

export const getVendorTerms = async (vendor: string): Promise<VendorTerms | null> => {
  const all = await getAllVendorTerms();
  const key = (vendor ?? "").trim().toLowerCase();
  return all.find((v) => v.vendor === key) ?? null;
};

export const computeQueuedPayments = (
  terms: VendorTerms,
  ctx: { poDate: string; poAmount?: number },
): QueuedPayment[] => {
  const dueDate = getVendorPaymentDueDate(terms.vendor, ctx.poDate);
  if (!dueDate) return [];
  return [
    {
      dueDate,
      amount: ctx.poAmount,
      blocksSend: terms.terms === "advance",
      label: `${formatVendorTerms(terms.terms)} — ${terms.name}`,
    },
  ];
};
