                ...
                >
                  ...
                  <div className="mt-1 space-y-1">
                    {events.slice(0, 3).map((event) => {
                      ...
                      return (
                        <div key={event.id} className="flex items-center gap-1 group">
                          <StatusIcon className={`w-2.5 h-2.5 shrink-0 ${statusConfig[event.status].color}`} />
                          <div className="flex items-center gap-1 min-w-0">
                            {event.platforms.map((p) => (
                              <div key={p} className={`w-1.5 h-1.5 rounded-full shrink-0 ${platformColors[p]}`} />
                            ))}
        ...
        <div className="xl:col-span-1 space-y-4">
            ...
            {selectedDayEvents.length > 0 ? (
              <div className="space-y-3">
                {selectedDayEvents.map((event) => {
                  ...
                  return (
                    <div key={event.id} className={`p-3 rounded-lg border ${cfg.bg} ${cfg.border}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <StatusIcon className={`w-3.5 h-3.5 ${cfg.color}`} />
                        <span className={`text-[10px] font-semibold uppercase tracking-wider ${cfg.color}`}>
                          {cfg.label}
                        </span>
                        <span className="text-[10px] text-dash-text-secondary ml-auto">{event.time}</span>
                      </div>
                      <p className="text-xs text-dash-text leading-relaxed mb-2">{event.title}</p>
              ...
              <div className="text-center py-6">
                ...
                <p className="text-xs text-dash-text-secondary">No posts on this day</p>
                <button
                  onClick={() => {
              ...
              <div className="text-center py-6">
                <Calendar className="w-8 h-8 mx-auto mb-2 text-dash-text-secondary/30" />
                <p className="text-xs text-dash-text-secondary">Click a day to see details</p>
              </div>
            )}
          </div>

          <div className="bg-dash-surface rounded-xl border border-dash-border p-5">
            <h3 className="text-sm font-semibold text-dash-text mb-4">Upcoming Scheduled</h3>
            <div className="space-y-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        {post.platforms.map((p) => (
                          <div key={p} className={`w-2 h-2 rounded-full ${platformColors[p]}`} />
                        ))}
          ...
          <div className="bg-dash-surface rounded-xl border border-dash-border p-4">
            <div className="space-y-2">
              {(Object.entries(platformColors) as [SocialPlatform, string][]).map(([platform, color]) => (
              ...
              <div className="pt-2 mt-2 border-t border-dash-border space-y-1.5">
                {(Object.entries(statusConfig) as [string, typeof statusConfig.scheduled][]).map(([key, cfg]) => {
                  const Icon = cfg.icon;
                  return (
                    <div key={key} className="flex items-center gap-2">
                      <Icon className={`w-3 h-3 ${cfg.color}`} />
                      <span className="text-xs text-dash-text-secondary">{cfg.label}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-dash-text-secondary border border-dash-border rounded-lg text-sm hover:bg-dash-bg transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
              Cancel
            </button>
