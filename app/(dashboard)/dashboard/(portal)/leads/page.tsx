...
export const formatPhoneOnly = (raw: string): string => {
  ...
  // For Mexico-style (+52 81 1234 4421): country + 2-digit area + ··· + last 4
  // Generic fallback: last 4 with the leading 4-5 digits exposed
  const last4 = onlyDigits.slice(-4);
  const lead = onlyDigits.slice(0, Math.min(onlyDigits.length - 4, 4));
  // If +52 + 10-digit MX number, show "+52 81 ··· 4421"
...
const columns = [
  ...
  columnHelper.accessor("source", {
    ...
    cell: (info) => {
      ...
      return <span className="text-xs text-dash-text-secondary">{v}</span>;
    },
  }),
