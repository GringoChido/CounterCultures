# Counter Cultures — MASTER PLAN  📦 *ARCHIVE — history + rules*

> **⚠️ 2026-05-26 — the ACTIVE plan you work off is now [`LEAN-ON-ODOO-PLAN.md`](LEAN-ON-ODOO-PLAN.md).** This file is the **archive**: the full milestone history (§10 / PM-1…PM-19), the Sacred Surface detail (§2), the cutover plan (§7), and the original operating rules (§0). **Nothing here is lost** — but day-to-day, follow the Lean plan.
...
>
> **🟠 NEW — 2026-05-26: the ACTIVE build plan moved to [`LEAN-ON-ODOO-PLAN.md`](LEAN-ON-ODOO-PLAN.md).** Roger reviewed the portal's quote/PO builders → we pivoted: **Odoo is the system of record for all transactions; the portal is a dashboard/read layer + "Open in Odoo" deep-links.** That new file supersedes **§1 + §6 below** (the old portal-first sprint) and reorganizes everything into two tracks — **A. Customer Storefront (the launch)** and **B. Staff Portal (dashboard over Odoo)** — plus **Workstream H**. **§0 / §2 / §7 / §10 here still apply.**
...
> **✅ WEEK-2 STATUS — 2026-05-26 (after the 05-25 build sprint; per-commit detail in §10 PM-2..PM-10; next-phase kickoff in [`docs/CONTINUATION-NEXT-PHASE.md`](docs/CONTINUATION-NEXT-PHASE.md)).**
>
> **DONE & pushed (`df92f56`, Netlify deploy GREEN):** canonical-domain (`ed0d5f3`) · base-URL unify (`ff477bb`) · dashboard-honesty/1b (`7c8c964`) · security-hardening/1a (`15c31f5`) · mobile catalog grid (`c8e28dd`) · design-refresh **Phase 1** (foundation + homepage hub, `348db3f`). The cheap correctness/security batch + domain fixes + mobile grid are CLOSED.
>
> **REMAINING in Week 2:**
> - **Design follow-on surfaces** (one Claude Code session each, inheriting `docs/design/DESIGN-PRINCIPLES.md`; driver = `docs/fixes/p1-design-refresh.md`): **catalog** (unblocked now the grid shipped — sequence after it, §0.5) · **PDP** (#2) · **brand/maker pages** (absorbs C3). **⚠️ GATED** on Joshua confirming the homepage design DIRECTION on the warm live site.
> - **Decisions/actions (Joshua):** catalog-scope (§8 #15 — recommend the ~4,236 merchandised SKUs) · Netlify env (set `NEXT_PUBLIC_SITE_URL`, delete `NEXT_PUBLIC_BASE_URL`) · confirm staging warms to fast loads (PM-10).
> - **Doc-hygiene (§1.5/E):** apply E2–E6 to §2/§0.4 · re-run the §9 archive migration (root cruft still present) · add `NEXT_PUBLIC_SITE_URL` + `NEXT_PUBLIC_ALLOW_INDEXING` to `.env.example` · `rm docs/fixes/p1-dashboard-honesty.md`.
> - **Workstream H (parallel — start now):** H1 (Google ownership transfer — longest pole) + H2 (credential inventory).
>
> **THEN → Week 3: the content pipeline** (largest launch lever — catalog is placeholder: prices `1.00`, ~0% EN/descriptions/images). Squarespace scrape run · Cloudflare Images + R2 (Joshua deferred ~2 wks — build the URL seam now) · CDN image migration · top-SKU AI descriptions (review-gated) · SEO technical pass. Full breakdown in the continuation brief.
...
- **2026-05-26 (PM-23 — Roger quote-PDF punchlist shipped + Odoo PDF download; 2 caveats flagged)** — Roger flagged the portal quote PDF on S01856 + S01852 (wrong/broken images, customer delivery-address rendering as a 0-qty/$0 product row, no IVA, payment-terms block, T&C/Notes backwards, text header not the logo). **Fix `a1de0a8` (LIVE + green, Netlify `6a1644ea`) — code-verified:** extracted shared `app/lib/quote-from-order.ts` + `quote-terms-block.tsx`; applied to BOTH templates (`quote-template.tsx` preview/doc-gen AND `quote-document.tsx` customer/print). Verified: image gated on `product-image-manifest.json` (no wrong/broken thumbs); note/section lines (qty0/price0/no-product) filtered out; IVA line from the order's `amount_tax`; canonical bilingual T&C from `docs/CC-STANDARD-QUOTE-TERMS.md`; payment-terms block conditional. **⚠️ CAVEAT 1 — fix #2 (logo) only PARTIAL:** header is still the text wordmark + address block (`quote-template.tsx:124-138`), NOT the actual CC logo image — no logo asset exists in the repo. Roger's "that's not our logo" is NOT fully resolved; needs Joshua to add the logo file to `public/brand/` then swap `<h1>`→`<img>`. (Also confirm the customer email actually drops — template still has the email guard at :161-163; helper should pass `""`.) **Fix `30ea8ad` (Odoo PDF download; committed, deploying) — ⚠️ CAVEAT 2:** calls `ir.actions.report._render_qweb_pdf` via `execute_kw` (`app/api/dashboard/odoo-report/route.ts:51-56`). Odoo's external API **blocks underscore-prefixed private methods**, so this will almost certainly raise → caught → `odoo_pdf_unavailable` → graceful toast "use Print Preview (Cmd+P)". Net: the Odoo-PDF-download won't return Odoo's PDF this way; the **reliable clean-PDF path is the now-fixed portal preview + Cmd+P**. If Odoo's own PDF is truly wanted over API, the working method is session-cookie auth (`/web/session/authenticate` → GET `/report/pdf/...`) — spec on request. Customer-404 fix `661b77b` confirmed LIVE earlier (deploy `6a163a52`). Active plan = `STATE-OF-THE-UNION.md`.
- **2026-05-26 (PM-22 — customers 404 cluster: root-caused + 2 fixes shipped, code-verified)** — Joshua hit "Customer not found / HTTP 404" on `/dashboard/customers/[id]` and an empty list. **Root cause (verified):** the customer screens read the `Odoo_Partners` Google-Sheet mirror tab (`odoo-sheets.ts:238-242` → `getCustomerProfile`/`getCustomerList`), but **nothing populated it** — the hourly Odoo→Sheets sync only mirrored `account.move`/`account.payment`/`sale.order`/`purchase.order` (`sync.ts:177-196`), no `res.partner`. `readSheet` returns `[]` (not a throw) on an empty tab (`dashboard-sheets.ts:146`), so partners were always empty → list blank, detail 404, and partner names blank on orders/invoices/vendors (all join via `getOdooPartners`). NOT an Odoo-login issue (portal uses `ODOO_API_KEY`, working — invoices were syncing). **Fix 1 — `fe0acbf` (deployed green, Netlify `6a1636bd`):** `fetchPartners()` live-reads `res.partner` when the mirror is empty (pattern from `e1f9e03`), + added `res.partner` to the hourly sync so the tab stays warm. Restored the list. **Fix 2 — `661b77b` (committed; NOT yet deployed — live deploy still `fe0acbf` as of this entry):** opening a customer from an order/invoice still 404'd because the link targets a specific partner id (`orders/[id]/page.tsx:314`, `invoices/[id]/page.tsx:604`) often a contact/address partner (rank 0, not a company) excluded by the bulk-fetch domain. Added `fetchPartnerById()` (full→`PARTNER_FIELDS_CORE` retry so a missing optional/MX field can't re-break it) and wired it into `getCustomerProfile`/`getVendorProfile` as a per-id fallback (+ cache push). **Cowork verified both diffs against code; list/join paths unchanged (parity).** Next: push + deploy `661b77b`, then Joshua smoke-tests (order/invoice → customer name → 360 loads, no 404). Fix files: `docs/fixes/p0-customers-odoo-partners-missing.md`, `docs/fixes/p0-customer-detail-fetch-by-id.md`. Active plan = `STATE-OF-THE-UNION.md`.
- **2026-05-26 (PM-21 — STATE OF THE UNION authored; new single source of truth)** — Deep, code-verified audit across all 6 touchpoints (six parallel deep-dive agents + manual verification of the highest-stakes claims). **New active driver = [`STATE-OF-THE-UNION.md`](STATE-OF-THE-UNION.md)**; `LEAN-ON-ODOO-PLAN.md` banner-superseded (pointer), this file remains the archive. **Headline finding (VERIFIED):** storefront card sales never reach Odoo — `handleCartPurchaseCompleted` (`dispatcher.ts:310–386`) writes Google Sheets + fires the rule engine but never creates the Odoo SO/invoice/payment; the Stripe→Odoo bridge (`dispatcher.ts:152–208`) only fires on `pi.metadata.odoo_invoice_id` (portal deal payments against an existing invoice) — directly contradicting `CART-RULES.md:40` (rule 21) and the lean-on-Odoo strategy. **Blockers logged:** B1 commerce-model decision + cart→Odoo bridge; B2 prod env-var landmines (`STRIPE_WEBHOOK_SECRET`, `ODOO_STRIPE_JOURNAL_ID`, `WHATSAPP_APP_SECRET`, `NEXT_PUBLIC_ALLOW_INDEXING`); B3 no named post-handoff technical owner; B4 ⚠️UNVERIFIED catalog prices (possible $1 placeholders masked by `>10` guards). **High:** CRM-Sheet ownership conflict (personal vs Shared Drive — AGENTS.md vs baseline), latent `public/robots.txt` regression (untracked, `Allow:/`), 5 routes hardcode the unverified prod email sender (quote-by-email broken in sandbox), no Squarespace→site 301 map. **Dashboard:** simplify pass speced (keep cockpit / deep-link / cut ~6 dead routes); stale baseline docs corrected (trade-program + sales-analytics fabrication already fixed in code). **7 open decisions (A–G) + the Workstream-H ownership register** captured in the new doc. Active plan = `STATE-OF-THE-UNION.md`.
- **2026-05-26 (PM-20 — handoff to a fresh Cowork task; immediate = fix the Odoo login before Roger's test note)** — This context is full; continuing in a new Cowork task. **Immediate priority:** smooth the Odoo-login experience (clicking "New Quote" deep-links to Odoo → unbranded generic login when not signed in; for Odoo-account holders it's a one-time login, session persists). Before sending Roger the test note: brand the Odoo login + enable "Sign in with Google" for `@countercultures.com.mx` + confirm a signed-in user lands on the new-quote form (Odoo-admin + Google-Cloud config, NOT portal code). **Open threads (now in LEAN-ON-ODOO-PLAN §2):** (1) does sales (Javier/Ian) have Odoo seats? — else give seats or a lean portal quote-create writing to Odoo via the service account; (2) deeper — do we even need most of the staff dashboard? most screens duplicate Odoo → could shrink Track B hard (the SIMPLIFY goal). **Roger test note saved** at `docs/Roger-update-note.md` (send AFTER the login fix). Active plan = `LEAN-ON-ODOO-PLAN.md`.
- **2026-05-26 (PM-19 — consolidated to ONE plan; dashboard-simplify kickoff staged; Roger update drafted)** — Per Joshua: work off ONE plan, lose nothing. **`LEAN-ON-ODOO-PLAN.md` is now the single self-contained active plan** (folded in §0 condensed rules + Sacred-Surface summary + §9 milestones); **this file (MASTER-PLAN.md) relabeled as the ARCHIVE** (full history §10, Sacred Surface §2, cutover §7 — all preserved). **Guiding principle locked: SIMPLIFY / lean on Odoo.** Next build stage kicked off: `docs/fixes/p1-dashboard-simplify-phase1.md` — trim the dashboard nav to the daily cockpit, demote dead/demo/unwired surfaces (email-campaigns, social, unwired analytics, broken drive sub-routes, design), flag the ambiguous ones (content-calendar, blog-manager, trade-program, customs, weekly-review, inbox) for Joshua's keep/cut. Joshua leads the visual redesign (storefront + kept dashboard elements) later. **Roger follow-up note** drafted (plain-language: what's fixed + 5 test steps + the scale-back-and-simplify message).
- **2026-05-26 (PM-18 — Roger-fix smoke test: deploy GREEN + code verified; catalog scope DECIDED, email PAUSED)** — **Smoke test of the pushed cluster:** Netlify deploy `6a160cef` is **`ready`/green**, commit_ref **`4913184`** (contains all 3 fixes), `error_message: null`, `plugin_state: success`, 175s, 5 functions (drip dormant — held; no 429). Code verified intact on the deployed commit: line-items live-read + parity guard + `loadQuoteDataFromOdoo` (`e1f9e03`); pivot odoo-links slug + hyphen domain + `OdooEditLink` wired (`84eb9a9`); customer-500 hook above early returns (`4913184`). **Live click-through is auth-gated** (portal = Google SSO `@countercultures.com.mx`; Claude doesn't SSO on Joshua's behalf) → Joshua has the 5-step manual checklist (order line-items render; customer-facing quote totals ≠ $0.00; customer page loads; New-Quote/Open-in-Odoo deep-links land + slug confirm). **Decisions:** catalog scope = **~4,236 merchandised SKUs** (LEAN-ON-ODOO-PLAN §7 #1, locked); email tool = **PAUSED, not blocking** (§7 #2). Active plan = `LEAN-ON-ODOO-PLAN.md`.
- **2026-05-26 (PM-17 — Roger cluster PUSHED `4913184`; new `LEAN-ON-ODOO-PLAN.md` is the active plan)** — All 3 commits pushed to `origin/main` (line-items `e1f9e03`, pivot `84eb9a9`, customer-500 `4913184`); **0 ahead.** **Reorganized around the lean-on-Odoo pivot:** new active driver **`LEAN-ON-ODOO-PLAN.md`** supersedes §1/§6 — two tracks (**A. Customer Storefront = the launch**; **B. Staff Portal = dashboard/read over Odoo**) + **Workstream H**. **Portal usefulness pass (Claude-led):** KEEP+SHARPEN the daily cockpit (overview, orders/customers/vendors read+detail, AR/AP for Antonina, invoices/payments, reports/P&L, leads + a Needs-Outreach queue, search, settings, the Odoo bridge); DEEP-LINK to Odoo for create/edit + deep transaction surfaces; CUT/DEFER email-campaigns (Klaviyo NO), social (demo), unwired analytics, the broken drive sub-routes, customs, trade-program admin, design/weekly-review/inbox (evaluate). Joshua leads the visual redesign (Track A3 storefront + the kept portal elements). **Dropped:** portal transaction builders + their T&C/image/attachment fixes + lead-pipeline-in-portal + trade Phase 2 as a portal build + fabricated analytics. **Open:** catalog scope (recommend 4,236) + email-tool replacement. §0/§2/§7/§10 unchanged.
- **2026-05-26 (PM-16 — Odoo pivot SHIPPED `84eb9a9` + customer-500 FIXED `4913184`; Roger cluster ready to push)** — **Pivot (`84eb9a9`):** 11 files, **146 ins / 1063 del** (net removal — `orders/new` −615, `purchases/new` −450 demoted to redirect CTAs). New Quote/PO/Customer/Vendor → Odoo create; "Open in Odoo" on order/PO/customer/vendor detail; `odoo-links.ts` has the hyphen domain + Odoo-17 slug pattern. Sacred Surface untouched; read/dashboard + line-items fix intact. **Customer-500 (`4913184`):** #5 was a Rules-of-Hooks violation — `useCurrentUser()` ran after the loading/error early returns in `customers/[id]/page.tsx`, changing the hook count loading→loaded → React crashed on **every** customer page. Fix: moved the hook to line 184, above both early returns (197/205); verified. **Roger quote cluster: #1/#2 line-items (`e1f9e03`) ✅ · create/edit pivot (`84eb9a9`) ✅ · #5 customer-500 (`4913184`) ✅.** Remaining: in-Odoo T&C template config + post-deploy slug verify. **3 commits unpushed** (`e1f9e03`, `84eb9a9`, `4913184`) — Joshua pushes (sandbox lacks push auth). ⚠️ Sandbox couldn't unlink git temp/lock files (`Operation not permitted`); if `git push` errors on a stale lock, clear `.git/*.lock` first. **NEXT: reorganize the plan around the Odoo pivot (task #13) — the portal-builder direction is superseded.**
- **2026-05-26 (PM-15 — Odoo deep-link URLs corrected in `odoo-links.ts`; held pivot ready to commit)** — Verify-pass on the pivot caught the deep-link URLs were guessed/wrong. Fixed directly (CoWork edit, same working tree as the held session): (1) **domain bug** — the `odoo-links.ts` fallback was `countercultures.odoo.com` (no hyphen) but `.env.local` `ODOO_URL` is `counter-cultures.odoo.com` (hyphen, confirmed) → corrected the fallback; every link was pointing at the wrong domain. (2) **pattern** — switched from `/odoo/{model}/{id}` + `?view_type=form` (old/raw-model) to the **Odoo-17 app-slug** form: `odooFormUrl=/odoo/{slug}/{id}`, `odooCreateUrl=/odoo/{slug}/new`, `odooListUrl=/odoo/{slug}` (slugs sales/purchase/contacts/accounting = Odoo defaults, matching the existing `/odoo/settings/users` link). Unauthenticated probing CAN'T validate slugs (Odoo preserves any `/odoo/*` path in the login redirect — even a deliberately-bogus one), so the exact slug words confirm only logged-in — a one-word fix if any differ. **Next:** held Claude Code session commits the pivot (now incl. this fix); set `NEXT_PUBLIC_ODOO_URL=https://counter-cultures.odoo.com` on Netlify; **rotate the Odoo password (pasted in-thread).**
- **2026-05-26 (PM-14 — DECISION: LEAN ON ODOO; quote/PO builders pivot to Odoo deep-links)** — Joshua resolved the PM-13 strategic question. **The portal stays the read/dashboard + light-actions layer; CREATE/EDIT of quotes, POs, customers, vendors moves to Odoo** (deep-linked), where lookup + create-with-full-fields, currency, IVA, discounts, images, and editable drafts already work. Confirms **Correction A** (Odoo = system of record). **Wiring confirmed:** `NEXT_PUBLIC_ODOO_URL=https://countercultures.odoo.com` (client-available) + an existing `components/odoo-link.tsx` to reuse; builders = `orders/new` + `purchases/new`; records carry Odoo ids. **Pivot fix-file written:** `docs/fixes/p0-pivot-create-edit-to-odoo.md` (reuse odoo-link → record/create deep-links; repoint New Quote/PO/Customer/Vendor; add "Open in Odoo" on detail pages; demote the builders to redirects; keep read/dashboard + the shipped line-items fix). **§0 YES recorded** (Joshua, 2026-05-26). **DROPPED as superseded** (stubbed "DO NOT RUN" — bash `rm` blocked by the mount): `p1-quote-terms-and-image.md`, `p1-quote-attachments.md`, the #2 edit path — create/edit is in Odoo now. **Kept:** #1 line-items (`e1f9e03`, makes the portal READ view correct), `p1-customer-detail-500.md` (#5, a crash — fix regardless). **New in-Odoo config task:** set the canonical bilingual T&C (`docs/CC-STANDARD-QUOTE-TERMS.md`) in Odoo's sale-order quote template. **Speed:** pivot ~1 session + crash fix ~1 session → Roger relief in ~2 sessions, and it DROPS 4 builder fix-files (less work, not more).
- **2026-05-26 (PM-13 — Roger round-2 feedback on the quote/PO builder → STRATEGIC DECISION raised; builder fix-cluster PAUSED)** — Roger reviewed the New Quote + New PO builders and is frustrated. Full punch list: **vendor lookup doesn't find existing Odoo vendors**; "create as new" can't capture a real partner (location, terms, website, contacts) — same for customers; **currency shows MXN when USD is selected**; **no IVA line; no discount; no product images; not editable day-to-day** (revisable drafts); layout isn't industry-standard; dashboard not user-friendly. **Meta-directive (the real point):** *"use industry standards, copy Odoo, don't start from zero — we have a working system."* Nearly every gap is something Odoo already does. **→ OPEN STRATEGIC DECISION (§8): how much should the portal REBUILD transaction creation vs. LEAN ON Odoo** (portal = read/dashboard + light actions; creation/editing of quotes/POs/customers/vendors deferred to / deep-linked into Odoo). Re-tests the "portal-first" direction against **Correction A** (Odoo stays the system of record). **Builder fix-cluster PAUSED pending a Roger meeting** — `p1-quote-terms-and-image.md` (#4/#6), `p1-quote-attachments.md` (#3), the #2 edit path. **Keep regardless:** #1 line-items (`e1f9e03`, shipped); #5 customer-500 (a crash — fix either way). Roger requested a Google meeting + provided Odoo read-only access for a flow walkthrough (recommend read-only, no edits; rotate the shared password after). **Recommend: meeting + decision BEFORE more builder code.**
- **2026-05-26 (PM-12 — quote line items #1/#2 FULLY FIXED `e1f9e03` + verified)** — Completed after a verify-pass caught the first pass only fixed the internal views, not the customer-delivered quote. Final (live-read, no cron change): `fetchSaleOrderLines()` live Odoo `sale.order.line` read (`odoo/client.ts`); orders API falls back to it when the mirror has 0 lines (`orders/[id]/route.ts:16` — guard: empty + `isConfigured`); **and `quote-data.ts` `loadQuoteDataFromOdoo` fallback** so the CUSTOMER lanes (public `/quote/[dealId]`, `quotes/[dealId]/print`, `send-quote`, `share`) render Odoo lines when there's no CRM Pipeline deal/items. **Parity verified:** the Odoo fallback fires ONLY when the CRM deal is null (`quote-data.ts:110`) or has 0 items (`:135`) — existing CRM-deal quotes + the cron/sync untouched. `S01858`: detail + customer quote now show 1 line, $509 subtotal / $590.44 total (was $0.00). Builder supports 2+ lines pre-save (#2 render half). Fix-file retired. **#1/#2 CLOSED.** 1 commit unpushed (`e1f9e03`). **Remaining quote cluster (queued):** `p1-quote-terms-and-image.md` (#4+#6) → `p1-customer-detail-500.md` (#5) → `p1-quote-attachments.md` (#3). "Edit lines on a saved quote" = noted follow-on.
- **2026-05-26 (PM-11 — Roger quote-flow bug cluster root-caused; 4 fix-files staged; JUMPS the queue ahead of the design follow-ons)** — Roger reported 6 bugs in the New Quote flow. Read-only root-cause (verified): **#1 line items don't render** — builder writes `order_line` to Odoo fine (`write.ts:252-274`) but the portal reads lines from the `Odoo_Sale_Order_Lines` mirror tab that NOTHING populates (no `sale.order.line` in the sync), and the customer PDF reads legacy CRM `Deal_Line_Items` (also empty) → header total shows, lines = 0, PDF $0.00 (e.g. `S01858` = $590.44 / 0 lines). **#2** = same root (post-save nav to read-only detail). **#3 attachments** read-only (no POST; `createAttachmentFor` exists, wired only to invoice-CFDI). **#4 T&C** = generic `DEFAULT_TERMS` placeholder (`orders/new:20`) + hardcoded PDF blocks (`quote-document.tsx:182-226`) — canonical bilingual terms captured in `docs/CC-STANDARD-QUOTE-TERMS.md`. **#5 customer-500** = Rules-of-Hooks violation (`useCurrentUser` after an early return in `customers/[id]/page.tsx`), triggered by a 404 when the partner isn't in the mirror. **#6 image** = never threaded through the line type/PDF. **Staged fix-files (run in order):** `p0-quote-line-items.md` (blocker — Odoo read/PDF; prefer live-read-per-order over touching the in-motion cron; §0 YES) → `p1-quote-terms-and-image.md` (#4 via the canonical doc + #6) → `p1-customer-detail-500.md` (#5) → `p1-quote-attachments.md` (#3). **§0 YES recorded 2026-05-26** for the quote-flow fixes. (T&C has a likely "suspected"→"subject" typo flagged for Joshua.)
- **2026-05-26 (PM-10 — deploy verified GREEN via Netlify; "slow loads" = cold-start, not a build failure)** — Joshua reported slow loads on `countercultures.netlify.app/en`. Checked Netlify directly (MCP): published deploy `6a150e60` is **state `ready`** (`error_message: null`), built from the push **`df92f56`**, **deploy_time 182s**, `plugin_state: success`; 5 functions deployed (server handler + fx-sync/keepalive/odoo-sync/stale-deal-sweep) — **`follow-up-drip` has NO schedule**, confirming the dormant-drip decision shipped. **The build-time 429 did NOT bite this deploy** → not the "no snapshot" scenario. **Conclusion: cold-start on a fresh deploy** — the ~48.6 MB / 1024 MB server handler cold-hydrates the 354K cache (~2.5s first hit per route; warm 50–280ms; the `*/3` keepalive cron rewarms). Normal if first-load-slow / refresh-fast; a runtime snapshot issue only if it stays 10s+ on warm loads (unlikely given the green build). Proper perf tuning (Core Web Vitals) is Week 5 / Concern 1.
- **2026-05-25 (PM-9 — FULL PUSH to `origin/main` `df92f56`)** — All 6 fix commits (`ed0d5f3` → `348db3f`) + the operating-doc commit (`df92f56`, this Master Plan) pushed; local `main` == `origin/main`, **0 ahead**. The day's work is on the remote (no longer laptop-only) and Netlify staging is deploying. Intentionally NOT pushed (local working tree): `.claude/launch.json`, `docs/audit/*.xlsx`, the §1.5/E5 cruft (`FULL-PASS-AUDIT.md`, the `… 2.md` dupes, `docs/superpowers/`, the Roger PDF), and the 2 kept fix-files (`p1-dashboard-honesty.md` retire-pending; `p1-design-refresh.md` = the follow-on driver). **Manual follow-ups now in Joshua's court:** (1) confirm the Netlify `main` deploy goes **GREEN** — the real test of whether the build-time 429 is local-only (prebuild snapshot, `20c3800`); (2) Netlify env — set `NEXT_PUBLIC_SITE_URL`, delete `NEXT_PUBLIC_BASE_URL` (cleanup; defaults already correct; leave `NEXT_PUBLIC_ALLOW_INDEXING` unset so staging stays noindex); (3) smoke `countercultures.netlify.app` EN+ES + catalog/search, and **eyeball the design refresh on desktop + phone — the §6 Week-2 design DECISION GATE that unblocks the catalog/PDP/brand follow-ons.**
